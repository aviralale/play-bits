...
import { BudgetGameState } from "@/types";

interface BudgetGameOverScreenProps {
  gameState: BudgetGameState;
  onRestart: () => void;
}

export default function BudgetGameOverScreen({
  gameState,
  onRestart,
}: BudgetGameOverScreenProps) {
  const router = useRouter();

  const averageScore =
    gameState.results.reduce((sum, result) => sum + result.score, 0) /
    gameState.results.length;

  const getRank = (score: number): string => {
    const averageScore = score / gameState.totalRounds;
    if (averageScore >= 600) return "Budget Master! 🏆";
    if (averageScore >= 450) return "Financial Wizard! 🌟";
    if (averageScore >= 300) return "Money Manager! 💰";
    if (averageScore >= 200) return "Budget Beginner! 📊";
    return "Learning to Budget! 🎯";
  };

  const maxPossibleScore = gameState.totalRounds * 700; // Approximate max score per round
      ...
      >
        ...
        >
          <div className="bg-gray-50 border-2 border-gray-900 rounded-lg p-4">
            ...
            <p className="text-xs text-gray-600 font-bold">Scenarios</p>
          </div>
          <div className="bg-gray-50 border-2 border-gray-900 rounded-lg p-4">
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <h3 className="font-black text-gray-900 mb-4">Scenario Results</h3>
          <div className="space-y-2">
            {gameState.results.map((result, index) => (
              ...
              >
                ...
                <span className="font-black text-blue-600 ml-4">
                  {result.score} pts
                </span>
              </div>
            ))}
